#!/usr/bin/env python3
"""Recheck the four named startup runs and write a compact, shareable report."""
from workflow import HERE, analyze, write_json


def main():
    runs = ["ion_coulomb_smoke200", "hybrid_smoke200", "kinetic_smoke200", "kinetic_half_dt_smoke400"]
    summaries = [analyze(HERE / "runs" / name) for name in runs]
    full, half = summaries[2:]
    electric_difference = abs(half["final_max_abs_Ex_V_m"] / full["final_max_abs_Ex_V_m"] - 1)
    electron_difference = abs(half["species"]["electrons"]["final_mean_energy_eV"] /
                              full["species"]["electrons"]["final_mean_energy_eV"] - 1)
    comparison = {
        "same_horizon": full["horizon_s"] == half["horizon_s"],
        "max_Ex_relative_difference": electric_difference,
        "electron_mean_energy_relative_difference": electron_difference,
        "coarse_startup_agreement_below_0.1_percent": electric_difference < 1e-3 and electron_difference < 1e-3,
        "not_a_convergence_claim": "Only one very short horizon, same spatial resolution; source macroparticle sampling changes with dt",
    }
    write_json(HERE / "RESULTS.json", {"date": "2026-09-10", "runs": summaries, "kinetic_timestep_comparison": comparison})
    rows = []
    for name, summary in zip(runs, summaries):
        beam = summary["species"]["protons"]
        horizon = "20 microseconds" if not summary["case"].startswith("kinetic") else "10 nanoseconds"
        rows.append(f"| `{name}` | {horizon} | {summary['execution']['wall_seconds']:.2f} s | "
                    f"{beam['final_count']:,} | {beam['final_mean_energy_eV']:.6f} | {summary['status']} |")
    text = """# Measured startup results - 2026-09-10
...
{sum(s['execution']['wall_seconds'] for s in summaries):.1f} seconds of solver time in total.
...
- Final maximum |Ex|: {full['final_max_abs_Ex_V_m']:.9f} versus {half['final_max_abs_Ex_V_m']:.9f} V/m.
- Relative difference of this field observable: {electric_difference:.6g} ({100*electric_difference:.6g} percent).
- Relative difference in final mean electron energy: {electron_difference:.6g}.
- Minimum sampled electron distance from a z end: {full['species']['electrons']['min_snapshot_z_seam_clearance_m']:.6f} m.
- Remaining particle plus electrostatic-field energy changes by approximately
  {full['remaining_particle_plus_E_energy_relative_change']:.6g} relative to its initial value.
...
"""
    ...
    print("Updated RESULTS.json and RESULTS.md")
    print(comparison)
    if not all(s["status"] == "integration_smoke_pass" for s in summaries):
        raise SystemExit("At least one named run needs review; inspect RESULTS.json")
